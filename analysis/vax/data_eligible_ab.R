...
eligibility_count <- tribble(
  ~description, ~n,
  "Extracted using study_definition_vax", n_distinct(data_vax_covs$patient_id)
)

################################################################################
cat("#### apply exclusion criteria from box a to processed data ####\n")
# remove dummy jcvi_group
data_eligible_a <- data_vax_covs %>%
