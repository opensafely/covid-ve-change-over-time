...
min_and_max_fu_dates <- readr::read_rds(
  here::here("output", "lib", "min_and_max_fu_dates.rds"))
...
plot_fun <- function(
  ...
  if (colour_var == "subgroup") {
    ...
    subtitle_string <- subgroups[plot_subgroup]
  }
  ...
  plot_res <- plot_data2 %>%
    ...
    geom_point(
      shape = point_shapes[plot_subgroup],
...
plot_res_all <- plot_grid(
  plot_res_list[[4]] + theme(legend.position = "none"),
  plot_res_list[[1]] + theme(legend.position = "none"),
  plot_res_list[[3]] + theme(legend.position = "none"),
  plot_res_list[[2]] + theme(legend.position = "none")
)


plot_grid(
  ,
  get_legend(plot_res_list[[1]]),
  nrow = 2,
  rel_heights = c(5,1)
)
...
grid.arrange(arrangeGrob(plot_res_all, left = y.grob, bottom = x.grob))


# save the plot
ggsave(filename = here::here("output", "models_cox", "images", glue("hr_{plot}.png")),
