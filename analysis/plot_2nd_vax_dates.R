...
plot_data_redacted <- readr::read_csv(
  here::here("output", "explore_2nd_vax_dates", "data", "plot_data_redacted.csv"),
  col_types = cols(
    region = col_character(),
    brand = col_character(),
    dose_2  = col_date(format = "%Y-%m-%d"),
    n = col_integer(),
...
for (plot_date in as.character(sort(unique(plot_data_redacted$elig_date)))) {
  ...
  age_group_info <- as.character(unique(data$age_group))
  ...
  data %>%
    ...
    labs(x = "date of second vaccination", y = "number of patients",
         ...
         caption = glue("X-axes show 6 to 14 weeks after eligibility date, with dashed line at eligibility date + 10 weeks. Bars are stacked by vaccine brand.")) +
