...
data_processed <- readr::read_rds(
  ...
  select(patient_id, subgroup,
         ...
         death_date, coviddeath_date, dereg_date)
...
data_all <- data_arm %>% 
  ...
  # BUT KEEP k=1 FOR BOTH, AS USED IN TABLE 1
  filter(
    is.na(split) |
      k == 1 |
      ((k %% 2) == 0 & split == "even") |
  ...
  mutate(across(end_k_date,
                ~ if_else(as.Date(study_parameters$end_date) < .x,
                          ...
                          .x))) %>%
  # find earliest evidence of prior COVID-19 
  # (can get away with earliest here as in 2021 (pre-omicron) multiple reinfections were very rare)
  mutate(
  ...
  # only keep events that occur during the comparison period
  mutate(across(c(all_of(str_c(outcomes, "_date")), dereg_date),
                ~if_else(
  ...
  select(patient_id, start_k_date, end_k_date, k,
         ...
         # outcomes and dereg
         all_of(str_c(outcomes, "_date")), dereg_date,
         # covariates
