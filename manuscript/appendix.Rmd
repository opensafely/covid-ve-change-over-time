...
event_counts <- bind_rows(
  readr::read_csv(here::here(release_folder, "event_counts_all.csv")),
  readr::read_csv(here::here(release_folder, "event_counts_6575.csv"))
  ) %>%
...
metareg_results_rhr <- readr::read_rds(
  ...
  mutate(across(outcome,
      ...
      mutate(across(comparison,
                  ...
                  levels = c("BNT162b2", "ChAdOx1", "both"),
                  labels = c("BNT162b2 vs unvaccinated", "ChAdOx1 vs unvaccinated", "BNT162b2 vs ChAdOx1"))) %>%
   mutate(across(c(rhr, rhr_lower, rhr_higher), 
                ~format(round(.x, digits=2), nsmall=2, trim = TRUE))) %>%
  select(subgroup, sex, ageband, comparison, outcome, rhr, rhr_lower, rhr_higher)
```
...
variable_defs <- local({
  ...
  "First occurence prior to SVP",
  #
  "JCVI group definitions",
  "Age",
  "-",
  "Numeric",
  "31 March 2021 for individuals eligible in phase 1 and 1 July 2021 for individuals eligible in phase 2",
  #
  "JCVI group definitions",
  "High risk",
  "A code in the primary care record indicating that the patient was at high risk of developing complications from COVID-19, not superseded by a code indicating that they were at low or moderate risk.",
  "0; 1.",
  "18 January 2021 (eligibility date for high risk individuals).",
  #
  "JCVI group definitions",
  "At-risk",
  "A code corresponding to one or more of the following: severe obesity (BMI 40+ kg/m\\textsuperscript{2}); learning disability including Down's syndrome; serious mental illness; chronic heart disease; chronic respiratory disease; chronic liver disease; chronic kidney disease; chronic neurological disease; diabetes; immunosuppressed (diagnosis of permenant immunosuppression or asplenia, or currently taking immunosuppresant medication).",
  "0; 1.",
  "15 February 2021 (eligibility date for at-rosk individuals).",
  #
  "Strata",
  "JCVI group",
  "Priority groups 2-9 are as defined by the JCVI expert advisory group, and we further define groups 10, 11 and 12 are defined as those aged 40-49, 30-39 and 18-29 years, respectively, who were not assigned to JCVI groups 4 (high risk) or 6 (at-risk).",
  "2-12",
  "-",
  #
  "Strata",
  ...
  "IMD quintile based on patient address. The IMD is a measure of socioeconomic deprivation based on lower-layer super output area (LSOA; a small geographical area defined by the Office of National Statistics). For more information see the  \\href{https://www.gov.uk/government/publications/english-indices-of-deprivation-2019-technical-report}{English IMD 2019: technical report}",
  "1; 2; 3; 4; 5.",
  "Eligibility date + 6 weeks ",
  #
  ...
  pack_rows_index <- sapply(data0_list, function(x) nrow(x))
  names(pack_rows_index) <- sapply(data0_list, function(x) unique(x$type))
  
  data <- bind_rows(data0_list) %>% select(-type) 
  ...
  data %>%
   kable(
      ...
      col.names = c("Name", "Description", "Encoding", "Date defined"),
      booktabs = TRUE, 
      longtable = TRUE,
    ...
    pack_rows(
      ...
      label_row_css = "border-bottom: 0.5px solid;",
...
```{r prep-table-events}
table_events_ft <- function(s="Both", a=FALSE) {
  ...
  if (s == "Both" & !a) {
    ...
    caption_string <- glue("{caption_string} ({str_to_lower(s)}s only).")
    ...
    data <- data0 %>%
      filter(ageband != "all") %>%
      mutate(
        subgroup = if_else(ageband == "65-74 years", "1", "2")) %>%
      select(-ageband)
    
    caption_string <- glue("{caption_string} (older adults).")
  ...
  table_out0 <- data %>%
    ...
    mutate(across(events, ~if_else(.x == "NA", "0", .x))) %>%
  ...
  keep <- cols_subtype != 0
  cols_subtype <- cols_subtype[keep]
  
  if (a) {
    
    names(cols_subtype) <- c("65-74 years", "75+ years")
  ...
  col_1_width <- 1
  col_2_width <- 0.8
  col_width <- (page_width - col_1_width - col_2_width - ncol(table_out0)*padding)/(ncol(table_out0) - 2)
  ...
  table_out1 <-  tibble(
    outcome = factor(
      rep(names(outcomes), each = length(weeks_since_2nd_vax)+1),
      levels = names(outcomes[outcomes_order])
    ),
    k = factor(
      rep(c(weeks_since_2nd_vax, "Total"), times = length(outcomes)),
      levels = c(weeks_since_2nd_vax, "Total"))
    ) %>%
    left_join(table_out0, by = c("outcome", "k")) %>%
    arrange(outcome, k) %>%
    select(outcome, k, everything()) %>%
    kable(
      format = "latex",
      col.names = col_names,
...
```{r prep-table-hrs}
table_hrs_ft <- function(m, s="Both", a=FALSE){
  ...
  if (s == "Both" & !a) {
    ...
    page_width <- 16
    s_groups <- c("65-74 years", "75+ years")
    
  }
  
  s_labs <- seq_along(s_groups)
  ...
  col_width <- (page_width - ncol(table_out0)*padding)/ncol(table_out0)
  col_width <- as.character(glue("{col_width}cm"))
  
  table_out1 <- tibble(
    outcome = factor(
      rep(names(outcomes), each = length(weeks_since_2nd_vax)),
      levels = names(outcomes[outcomes_order])
    ),
    k = factor(
      rep(weeks_since_2nd_vax, times = length(outcomes)),
      levels = weeks_since_2nd_vax)
    ) %>%
    left_join(table_out0, by = c("outcome", "k")) %>%
    arrange(outcome, k) %>%
    select(all_of(table_names)) %>%
    mutate(across(starts_with(as.character(s_labs)),
                  ~ if_else(
                    is.na(.x),
                    "-",
                    .x)
...
tables_data <- names_key %>%
  ...
  select(comparison, subgroup, sex, ageband, outcome, period, variable = names_long, label, value) %>%
  ...
  mutate(across(starts_with(weeks_since_2nd_vax), ~if_else(is.na(.x), "-", .x)))  

table_coef_hrs <- function(comp, sub, s = "Both", a = "all", o){
  ...
  data <- tables_data %>%
    ...
    select(-comparison, -subgroup, -sex, -ageband, -outcome)
  ...
  caption_string <- glue("Covariate hazard ratios for {names(outcomes)[outcomes == o]} in the {comp} comparison and the {subgroups[sub]} subgroup")
  ...
  table_out1 <- table_out0 %>%
    ...
    footnote(
      general = c("Estimates correspond to a 1-year increase in age (or age-squared as indicated) within the given age categories."),
      general_title = "*",
      footnote_as_chunk=TRUE,
      escape=FALSE
    )
...
```{r svp-02-2020-12-08, include=TRUE, fig.cap="Second vaccination period for JCVI group 2", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_02_2020-12-08.png"))
```

```{r svp-03-2020-12-08, include=TRUE, fig.cap="Second vaccination period for JCVI group 3", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_03_2021-01-18.png"))
```

```{r svp-04a-2021-01-18, include=TRUE, fig.cap="Second vaccination period for JCVI group 4", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_04a_2021-01-18.png"))
```

```{r svp-04b-2021-01-18, include=TRUE, fig.cap="Second vaccination period for JCVI group 4", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_04b_2021-01-18.png"))
```

```{r svp-05-2021-02-15, include=TRUE, fig.cap="Second vaccination period for JCVI group 5", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_05_2021-02-15.png"))
```

```{r svp-06-2021-02-15, include=TRUE, fig.cap="Second vaccination period for JCVI group 6", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_06_2021-02-15.png"))
```

```{r svp-07-2021-02-22, include=TRUE, fig.cap="Second vaccination period for JCVI group 7 and aged 64 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_07_2021-02-22.png"))
```

```{r svp-07-2021-03-01, include=TRUE, fig.cap="Second vaccination period for JCVI group 7 and aged 60-63 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_07_2021-03-01.png"))
```

```{r svp-08-2021-03-08, include=TRUE, fig.cap="Second vaccination period for JCVI group 8", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_08_2021-03-08.png"))
```

```{r svp-09-2021-03-19, include=TRUE, fig.cap="Second vaccination period for JCVI group 9", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_09_2021-03-19.png"))
```

```{r svp-10-2021-04-13, include=TRUE, fig.cap="Second vaccination period for JCVI group 10 and aged 45-49 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_10_2021-04-13.png"))
```

```{r svp-10-2021-04-26, include=TRUE, fig.cap="Second vaccination period for JCVI group 10 and aged 40-44 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_10_2021-04-26.png"))
```

```{r svp-11-2021-05-13, include=TRUE, fig.cap="Second vaccination period for JCVI group 11 and aged 36-39 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_11_2021-05-13.png"))
```

```{r svp-11-2021-05-21, include=TRUE, fig.cap="Second vaccination period for JCVI group 11 and aged 30-35 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_11_2021-05-21.png"))
```

```{r svp-12-2021-06-08, include=TRUE, fig.cap="Second vaccination period for JCVI group 12 and aged 25-29 years", out.width="0.85\\linewidth"}
knitr::include_graphics(file.path(release_folder, "svp_plots", "plot_by_region_12_2021-06-08.png"))
```

```{r svp-12-2021-06-15, include=TRUE, fig.cap="Second vaccination period for JCVI group 12 and aged 18-24 years", out.width="0.85\\linewidth"}
...
The distribution of follow-up time in relation to virus variants is given in \figsref{fig:check-fu-65} to \ref{fig:check-fu-18-39}.
We used 1 June 2021 as the approximate date at which the Delta variant became dominant in England ([BMJ 2021;373:n1445](https://doi.org/10.1136/bmj.n1445)), 1 December 2021 as the date at which the first cases of Omicron were detected, and 15 December 2021 as the date at which Omicron became dominant ([Omicron daily overview: 17 December 2021](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/1042100/20211217_OS_Daily_Omicron_Overview.pdf)).

```{r check-fu-65, include=TRUE, fig.cap="Distribution of follow-up time across the six comparison periods in the 65+ years subgroup. Comparison periods (numeric labels on right-hand side of plots) correspond to the following weeks since second vaccine dose: 1 = 3-6 weeks; 2 = 7-10 weeks; 3: 11-14 weeks; 4: 15-18 weeks; 5: 19-22 weeks; 6 = 23-26 weeks.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "variants", "check_fu_65+ years.png"))
```

```{r check-fu-18-64, include=TRUE, fig.cap="Distribution of follow-up time across the six comparison periods in the 18-64 years and clinically vulnerable subgroup. Comparison periods (numeric labels on right-hand side of plots) correspond to the following weeks since second vaccine dose: 1 = 3-6 weeks; 2 = 7-10 weeks; 3: 11-14 weeks; 4: 15-18 weeks; 5: 19-22 weeks; 6 = 23-26 weeks.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "variants", "check_fu_18-64 years and clinically vulnerable.png"))
```

```{r check-fu-40-64, include=TRUE, fig.cap="Distribution of follow-up time across the six comparison periods in the 40-64 years subgroup. Comparison periods (numeric labels on right-hand side of plots) correspond to the following weeks since second vaccine dose: 1 = 3-6 weeks; 2 = 7-10 weeks; 3: 11-14 weeks; 4: 15-18 weeks; 5: 19-22 weeks; 6 = 23-26 weeks.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "variants", "check_fu_40-64 years.png"))
```

```{r check-fu-18-39, include=TRUE, fig.cap="Distribution of follow-up time across the six comparison periods in the 18-39 years subgroup. Comparison periods (numeric labels on right-hand side of plots) correspond to the following weeks since second vaccine dose: 1 = 3-6 weeks; 2 = 7-10 weeks; 3: 11-14 weeks; 4: 15-18 weeks; 5: 19-22 weeks; 6 = 23-26 weeks.", out.width="\\linewidth"}
...
This subsection provides additional results for the waning of vaccine effectiveness across the four risk-based subgroups.
\tabref{tab:table-events} gives the event counts / person-years for each subgroup, outcome and comparison period; \tabsref{tab:table-hrs-ft} and \ref{tab:table-ahrs-ft} give the unadjusted and adjusted hazard ratios (respectively) for each subgroup, outcome and comparison period; \tabref{tab:table-ratio-hrs-ft} gives the per-comparison-period ratio of adjusted hazard ratios.
The unadjusted and adjusted hazard ratios are also plotted in \figsref{fig:hr-vax-BNT162b2}, \ref{fig:hr-vax-ChAdOx1} and \ref{fig:hr-vax-both} for BNT162b2 vs unvaccinated, ChAdOx1 vs unvaccinated and BNT162b2 vs ChAdOx1 respectively.
The unadjusted and adjusted hazard ratios with any SARS-CoV-2 test as the outcome are plotted in \figsref{fig:hr-vax-anytest-BNT162b2}, \ref{fig:hr-vax-anytest-ChAdOx1} and \figref{fig:hr-vax-anytest-both} for BNT162b2 vs unvaccinated, ChAdOx1 vs unvaccinated and BNT162b2 vs ChAdOx1 respectively.
The adjusted hazard ratios and lines corresponding to the ratio of hazard ratios with any SARS-CoV-2 test as the outcome are plotted in \figref{fig:hr-vax-anytest} for BNT162b2 vs unvaccinated and ChAdOx1 vs unvaccinated, and \figref{fig:hr-brand-anytest} for BNT162b2 vs ChAdOx1.
The hazard ratios corresponding to the covariates in the adjusted models are given in \tabsref{tab:coef-BN-1-B-covidadmitted} to \ref{tab:coef-BN-4-B-anytest}.
...
```{r hr-vax-ChAdOx1, include=TRUE, fig.cap="Unadjusted and adjusted hazard ratios for ChAdOx1 vs unvaccinated", out.width="\\linewidth"}
...
```{r hr-vax-anytest-ChAdOx1, include=TRUE, fig.cap="Unadjusted and adjusted hazard ratios for any SARS-CoV-2 test for ChAdOx1 vs unvaccinated", out.width="\\linewidth"}
...
```{r coef-BN-1-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
...
```{r coef-BN-1-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 1,
...
```{r coef-BN-1-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 1,
...
```{r coef-BN-1-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 1,
...
```{r coef-AZ-1-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 1,
    s = "Both",
    o = "covidadmitted"
    ))
...
```{r coef-AZ-1-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 1,
...
```{r coef-AZ-1-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 1,
...
```{r coef-AZ-1-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 1,
...
```{r coef-AZ-1-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 1,
...
```{r coef-both-1-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 1,
    s = "Both",
    o = "covidadmitted"
    ))
...
```{r coef-both-1-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 1,
...
```{r coef-both-1-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 1,
...
```{r coef-both-1-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 1,
...
```{r coef-both-1-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 1,
...
```{r coef-BN-2-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 2,
...
```{r coef-BN-2-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 2,
...
```{r coef-BN-2-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 2,
...
```{r coef-BN-2-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 2,
...
```{r coef-BN-2-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 2,
...
```{r coef-AZ-2-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 2,
...
```{r coef-AZ-2-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 2,
...
```{r coef-AZ-2-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 2,
...
```{r coef-AZ-2-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 2,
...
```{r coef-AZ-2-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 2,
...
```{r coef-both-2-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 2,
...
```{r coef-both-2-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 2,
...
```{r coef-both-2-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 2,
...
```{r coef-both-2-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 2,
...
```{r coef-both-2-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 2,
...
```{r coef-BN-3-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 3,
...
```{r coef-BN-3-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 3,
...
```{r coef-AZ-3-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 3,
...
```{r coef-AZ-3-B-coviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 3,
...
```{r coef-AZ-3-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 3,
...
```{r coef-AZ-3-B-noncoviddeath, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 3,
...
```{r coef-AZ-3-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "ChAdOx1 vs unvaccinated",
    sub = 3,
...
```{r coef-both-3-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 3,
...
```{r coef-both-3-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs ChAdOx1",
    sub = 3,
...
```{r coef-BN-4-B-covidadmitted, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 4,
...
```{r coef-BN-4-B-postest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 4,
...
```{r coef-BN-4-B-anytest, include=TRUE}
landscape(
  table_coef_hrs(
    comp = "BNT162b2 vs unvaccinated",
    sub = 4,
...
This subsection provides additional results for the waning of vaccine effectiveness across eight subgroups: the four risk-based subgroups, each split into females and males.
\tabsref{tab:table-events-female}  and \ref{tab:table-events-male} give the event counts / person-years for each subgroup, outcome and comparison period for females and males respectively.
\tabsref{tab:table-hrs-ft-female} and \ref{tab:table-hrs-ft-male} give the unadjusted hazard ratios for females and males respectively.
\tabsref{tab:table-ahrs-ft-female} and \ref{tab:table-ahrs-ft-male} give the adjusted hazard ratios for females and males respectively.
\tabsref{tab:table-ratio-hrs-ft-female} and \ref{tab:table-ratio-hrs-ft-male} give the per-comparison-period ratio of adjusted hazard ratios for females and males respectively.
The adjusted hazard ratios are plotted in \figsref{fig:hr-vax-BNT162b2-sex} and \ref{fig:hr-vax-ChAdOx1-sex} for BNT162b2 vs unvaccinated and ChAdOx1 vs unvaccinated respectively.
The adjusted hazard ratios with any SARS-CoV-2 test as the outcome are plotted in \figsref{fig:hr-vax-anytest-BNT162b2-sex} and \ref{fig:hr-vax-anytest-ChAdOx1-sex} for BNT162b2 vs unvaccinated and ChAdOx1 vs unvaccinated respectively.
...
```{r hr-vax-BNT162b2-sex, include=TRUE, fig.cap="Adjusted hazard ratios for BNT162b2 vs unvaccinated from the female- and male-only models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_BNT162b2_sex.png"))
```

```{r hr-vax-ChAdOx1-sex, include=TRUE, fig.cap="Adjusted hazard ratios for ChAdOx1 vs unvaccinated from the female- and male-only models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_ChAdOx1_sex.png"))
```

\end{landscape}

```{r hr-vax-anytest-BNT162b2-sex, include=TRUE, fig.cap="Adjusted hazard ratios for any SARS-CoV-2 test for BNT162b2 vs unvaccinated from the female- and male-only models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_anytest_BNT162b2_sex.png"))
```

```{r hr-vax-anytest-ChAdOx1-sex, include=TRUE, fig.cap="Adjusted hazard ratios for any SARS-CoV-2 test for ChAdOx1 vs unvaccinated from the female- and male-only models.", out.width="\\linewidth"}
...
This subsection provides additional results for the waning of vaccine effectiveness across two additional subgroups: 65-74 years and 75+ years.
\tabref{tab:table-events-ageband} gives the event counts / person-years for each subgroup, outcome and comparison period.
\tabsref{tab:table-hrs-ft-ageband} and \ref{tab:table-ahrs-ft-ageband} give the unadjusted and adjusted hazard ratios respectively.
\tabref{tab:table-ratio-hrs-ft-female} gives the per-comparison-period ratio of adjusted hazard ratios.
The adjusted hazard ratios are plotted in \figsref{fig:hr-vax-BNT162b2-ageband} and \ref{fig:hr-vax-ChAdOx1-ageband} for BNT162b2 vs unvaccinated and ChAdOx1 vs unvaccinated respectively.
The adjusted hazard ratios with any SARS-CoV-2 test as the outcome are plotted in \figsref{fig:hr-vax-anytest-BNT162b2-ageband} and \ref{fig:hr-vax-anytest-ChAdOx1-ageband} for BNT162b2 vs unvaccinated and ChAdOx1 vs unvaccinated respectively.

```{r table-events-ageband, include=TRUE, tab.cap.pre = "Supplementary Table "}
table_events_ft(a=TRUE)
```

\newpage
```{r table-hrs-ft-ageband, include=TRUE, tab.cap.pre = "Supplementary Table "}
table_hrs_ft(m = "unadjusted", a=TRUE)
```

\newpage
```{r table-ahrs-ft-ageband, include=TRUE, tab.cap.pre = "Supplementary Table "}
table_hrs_ft(m = "adjusted", a=TRUE)
```

```{r table-ratio-hrs-ft-ageband, include=TRUE, tab.cap.pre = "Supplementary Table "}
table_metareg_ft(a = TRUE)
```

```{r hr-vax-BNT162b2-ageband, include=TRUE, fig.cap="Adjusted hazard ratios for BNT162b2 vs unvaccinated from the 65-74 years and 75+ years models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_BNT162b2_ageband.png"))
```

```{r hr-vax-ChAdOx1-ageband, include=TRUE, fig.cap="Adjusted hazard ratios for ChAdOx1 vs unvaccinated from the 65-74 years and 75+ years models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_ChAdOx1_ageband.png"))
```

```{r hr-vax-anytest-BNT162b2-ageband, include=TRUE, fig.cap="Adjusted hazard ratios for any SARS-CoV-2 test for BNT162b2 vs unvaccinated from the 65-74 years and 75+ years models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_anytest_BNT162b2_ageband.png"))
```

```{r hr-vax-anytest-ChAdOx1-ageband, include=TRUE, fig.cap="Adjusted hazard ratios for any SARS-CoV-2 test for ChAdOx1 vs unvaccinated from the 65-74 years and 75+ years models.", out.width="\\linewidth"}
knitr::include_graphics(file.path(release_folder, "hr_vax_anytest_ChAdOx1_ageband.png"))
```
