...
```{r}
# # derive event counts data
# event_counts <- estimates_all %>%
#   filter(
#     variable == "k",
#     model == "unadjusted",
#     comparison != "BNT162b2 vs ChAdOx1"
#   ) %>%
#   select(
#     subgroup, sex, comparison, outcome, period, label, 
#     n_event_label, persondays
#   ) %>%
#   mutate(arm = if_else(
#     label == "0",
#     "Unvaccinated",
#     str_remove(comparison, " vs unvaccinated")
#   )) %>%
#   select(-comparison, -label) %>%
#   rename(k=period, events=n_event_label) %>%
#   distinct() %>%
#   mutate(person_years = round(persondays/365, 0)) %>%
#   select(-persondays)
```
...
table_metareg_ft <- function(s) {
  ...
  caption_string <- "Ratio of adjusted hazard ratios for each comparison period"
  if (s == "Both") {
    caption_string <- glue("{caption_string}.")
  } else {
    caption_string <- glue("{caption_string} ({str_to_lower(s)} only model).")
...
#                 labels = subgroups[subgroup_labels])) %>%
