...
variable_defs <- local({
  ...
  "2-12",
  "Age defined on 31 March 2021 for individuals eligible in phase 1 and 1 July 2021 for individuals eligible in phase 2. Individuals defined as clinically extremely vulnerable on 18 Januray 2021, and at-risk on 15 February 2021.",
  #
  "Strata",
  "Eligibility date",
  ...
  data %>%
   kable(
    ...
    footnote(
      general = c("JCVI: Joint Committee on Vaccination and Immunisation; start\\\\textsubscript{ki} is the start date of comparison period \\\\textit{k} for individual \\\\textit{i}.\\\\newline"),
      general_title = "",
...
print_table1 <- local({
  ...
  col_names <- str_replace(col_names, "ChAdOx", "ChAdOx1")
...
table_estimates_covs_ft <- function(i) {
  ...
  caption_string <- glue("Hazard ratios corresponding to covariates in the adjusted {comparison} model in the {subgroup_longer} subgroup.")
  ...
  # add column for missing outcomes for consistency across tables
  missing_outcomes <- !(names(outcomes) %in% names(table_out0))
  if (any(missing_outcomes)) {
    for (o in names(outcomes)[missing_outcomes]) {
    table_out0 <- table_out0 %>%
      mutate(!! sym(o) := NA_character_)
    }
  }
...
tribble(
  ~priority_group, ~description,
  "1", "Residents in a care home for older adults",
  "1", "Staff in a care home for older adults",
  "2", "All those 80 years of age and over",
  "2", "Frontline health and social care workers",
  "3", "All those 75 years of age and over",
  "4", "All those 70 years of age and over",
  "4", "Individuals aged 16 to 69 in a high risk group\\textsuperscript{a}",
  "5", "All those 65 years of age and over",
  "6", "Individuals aged 16 to 65 years in an at-risk group\\textsuperscript{a}",
  ...
  kable(
      format = "latex",
      caption = "Priority groups for primary vaccination advised by the Joint Committee on Vaccination and Immunisation",
      col.names = c("Priority group", "Risk group"),
      booktabs = TRUE, 
      longtable = TRUE,
      escape = FALSE # manually escape all special characters
    ) %>%
...
tribble(
    ...
    "2020-12-08", "2020-12-08", "-", "01, 02, 03", "-",
    "2021-01-18", "2021-01-18", "-", "04", "https://www.bbc.co.uk/news/uk-55698132",
    ###
    "2021-02-15", "2021-02-15", "-", "05, 06", "https://www.england.nhs.uk/statistics/wp-content/uploads/sites/2/2021/02/COVID-19-weekly-announced-vaccinations-25-February-2021.pdf",
    ###
    "2021-02-22", "2021-02-22", "64", "07", "https://www.england.nhs.uk/2021/03/nhs-invites-people-aged-60-plus-to-book-life-saving-covid-jab/", 
    "2021-03-01", "2021-03-01", "60-63", "07", "https://www.england.nhs.uk/2021/03/nhs-invites-people-aged-60-plus-to-book-life-saving-covid-jab/",
    ###
    # combine 2 rows as < 7 days between elig_dates
    "2021-03-08", "2021-03-08", "56-59", "08", "https://www.england.nhs.uk/2021/03/56-59/",
    "2021-03-08", "2021-03-09", "55", "08", "https://web.archive.org/web/20210309005613/https://www.nhs.uk/conditions/coronavirus-covid-19/coronavirus-vaccination/who-can-get-the-vaccine/",
    ###
    "2021-03-19", "2021-03-19", "50-54", "09", "https://web.archive.org/web/20210319164659/https://www.nhs.uk/conditions/coronavirus-covid-19/coronavirus-vaccination/who-can-get-the-vaccine/",
    ###
...
knitr::include_graphics(file.path(release_folder, "hr_vax_ChAdOx1.png"))
```
...
knitr::include_graphics(file.path(release_folder, "hr_vax_anytest_ChAdOx1.png"))
```
...
```{r table-estimates-covs-3, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(3))
```

```{r table-estimates-covs-4, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(4))
```

```{r table-estimates-covs-5, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(5))
```

```{r table-estimates-covs-6, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(6))
```

```{r table-estimates-covs-7, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(7))
```

```{r table-estimates-covs-8, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(8))
```

```{r table-estimates-covs-9, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(9))
```

```{r table-estimates-covs-10, include=TRUE, tab.cap.pre = "Supplementary Table "}
landscape(table_estimates_covs_ft(10))
```
